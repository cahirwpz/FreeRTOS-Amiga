/* command.c, Zepto Emacs, Public Domain, Hugh Barney, 2017, Derived from:
...
void lnbegin() {
  curbp->b_point =
      segstart(curbp, lnstart(curbp, curbp->b_point), curbp->b_point);
}
void quit() { done = 1; }
...
void copy_cut(int cut) {
  ...
  if ((scrap = (char_t *)malloc(nscrap)) == NULL) {
    ...
    if (cut) {
      ...
      curbp->b_point =
          pos(curbp, curbp->b_egap); /* set point to after region */
      curbp->b_flags |= B_MODIFIED;
