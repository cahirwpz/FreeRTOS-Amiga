/* raw.c, Zepto Emacs, Public Domain, Hugh Barney, 2017, Derived from: Anthony's
 * Editor January 93
...
char getch() { char ch = 0; read(STDIN_FILENO, &ch, 1); return ch; }
void flushinp() { }

/* Simple buffer structure to append all output to write in one go by calling
 * refresh * This avoids flickering. */
struct screen_buf {
  char *buf;
  int len;
};
...
void move(int line, int col) {
  char buf[32];
  snprintf(buf, sizeof(buf), GOTOXY, line + 1, col + 1);
  addstr(buf);
}
