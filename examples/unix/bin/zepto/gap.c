/* gap.c, Zepto Emacs, Public Domain, Hugh Barney, 2017, Derived from: Anthony's
 * Editor January 93 */
...
void save() {
  ...
  fd = open(curbp->b_fname, O_WRONLY);
  if (fd == -1)
    msg("Failed to open file \"%s\".", curbp->b_fname);
  (void)movegap(curbp, (point_t)0);
  length = (point_t)(curbp->b_ebuf - curbp->b_egap);
  if (write(fd, curbp->b_egap, sizeof(char) * (size_t)length) != length)
    msg("Failed to write file \"%s\".", curbp->b_fname);
  close(fd);
  curbp->b_flags &= ~B_MODIFIED;
  msg("File \"%s\" %ld bytes saved.", curbp->b_fname,
      pos(curbp, curbp->b_ebuf));
}
...
int insert_file(char *fn, int modflag) {
  ...
  if (curbp->b_egap - curbp->b_gap < (long)(sb.st_size * sizeof(char_t)) &&
  ...
  curbp->b_gap += len =
      read(fd, curbp->b_gap, sizeof(char) * (size_t)sb.st_size);
