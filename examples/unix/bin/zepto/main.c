/* main.c, Zepto Emacs, Public Domain, Hugh Barney, 2017, Derived from:
...
void die(char *msg, ...) {
  va_list args;
  va_start(args, msg);
  (void)vdprintf(STDERR_FILENO, msg, args);
  va_end(args);
  exit(1);
}
...
void msg(char *msg, ...) {
  va_list args;
  va_start(args, msg);
  (void)vsnprintf(msgline, TEMPBUF, msg, args);
  va_end(args);
  msgflag = TRUE;
}
